...
* `49-socketchannel2.lua`
  ...
  local channel = sc.channel{
    host = "127.0.0.1",
    port = 8001,
    response = dispatch --dispatch函数需要在channel对象之前定义
  }
  ...
  local function task()
    ...
    while i < 3 do
      skynet.fork(function(session)
        recv = channel:request("data "..session.."\n",session) --发送请求,并等待响应返回
        skynet.error("client recv",recv,session)
      end,i)
      i = i + 1
    end
  end
...
* 控制台输出
  ...
  [:01000010] listen 0.0.0.0:8001
  49-socketchannel2
  [:01000012] LAUNCH snlua 49-socketchannel2
  [:01000010] 127.0.0.1:51050 accepted
  [:01000010] service recv data 0
  [:01000010] service recv data 1
  [:01000010] service recv data 2
  [:01000012] client recv DATA 1 1 #不同协程的返回顺序是不一样的,因为fork之后不同session不用阻塞等待
  [:01000012] client recv DATA 2 2
  [:01000012] client recv DATA 0 0
  ```
