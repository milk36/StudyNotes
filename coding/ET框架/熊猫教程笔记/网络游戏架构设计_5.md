...
- [网络游戏架构设计 笔记五](#网络游戏架构设计-笔记五)
  - [Actor](#actor)
    - [ET Actor](#et-actor)
    - [ET 中 Protobuf 消息类型及用途](#et-中-protobuf-消息类型及用途)
    - [Actor 协议消息](#actor-协议消息)
    - [ActorLocation 协议消息](#actorlocation-协议消息)
    - [ActorMessage 协议消息](#actormessage-协议消息)
  - [ActorMessageSenderComponent 发送消息](#actormessagesendercomponent-发送消息)
    - [ProcessActorId 进程ID解析/封装](#processactorid-进程id解析封装)
    - [Send 无返回请求](#send-无返回请求)
    - [Call 有返回请求](#call-有返回请求)
    - [ActorMessageSender 关联发送与回调返回逻辑](#actormessagesender-关联发送与回调返回逻辑)
    - [HandleIActorResponse 处理Actor回调消息](#handleiactorresponse-处理actor回调消息)
    - [Check 检测RCP Callback调用超时定时器](#check-检测rcp-callback调用超时定时器)
  - [ActorHandleHelper](#actorhandlehelper)
    - [HandleIActorResponse](#handleiactorresponse)
    - [HandleIActorRequest](#handleiactorrequest)
    - [HandleIActorMessage](#handleiactormessage)
    - [Reply 发送响应消息](#reply-发送响应消息)
    - [MailboxComponent](#mailboxcomponent)
  - [ActorMessageDispatcherComponent 接收处理消息](#actormessagedispatchercomponent-接收处理消息)
    - [Load 初始加载消息处理器](#load-初始加载消息处理器)
    - [Handle 消息分发处理](#handle-消息分发处理)
    - [AMActorHandler 无返回值](#amactorhandler-无返回值)
    - [AMActorRpcHandler 有返回值](#amactorrpchandler-有返回值)
    - [Game WaitFrameFinish 在逻辑帧最后执行](#game-waitframefinish-在逻辑帧最后执行)
    - [对端建立两个Session的原因](#对端建立两个session的原因)
  - [Actor Location](#actor-location)
    - [ActorLocation 节点间通信流程图](#actorlocation-节点间通信流程图)
    - [LocationComponent 位置中心服务](#locationcomponent-位置中心服务)
    - [Lock 锁定 instanceId](#lock-锁定-instanceid)
    - [UnLock 解锁 instanceId](#unlock-解锁-instanceid)
    - [Add Remove Get 操作](#add-remove-get-操作)
    - [LocationProxyComponent 访问位置服务代理](#locationproxycomponent-访问位置服务代理)
    - [ActorLocationSenderComponentSystem 邮差](#actorlocationsendercomponentsystem-邮差)
    - [Send](#send)
    - [Call](#call)
    - [CallInner](#callinner)
