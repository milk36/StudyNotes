...
- [网络游戏架构设计 笔记三](#网络游戏架构设计-笔记三)
  - [序列化与反序列化](#序列化与反序列化)
    - [Entity 序列化的判断](#entity-序列化的判断)
    - [DeserializeSystem 反序列化后执行的System](#deserializesystem-反序列化后执行的system)
  - [网络层](#网络层)
    - [基础知识](#基础知识)
    - [ET的网络需求](#et的网络需求)
    - [ET的网络设计](#et的网络设计)
    - [主线程与网络线程的结构图](#主线程与网络线程的结构图)
    - [NetServices 线程安全队列](#netservices-线程安全队列)
    - [NetThreadComponent 网络线程组件](#netthreadcomponent-网络线程组件)
    - [主线程与网络线程的跨线程交互 源码解析](#主线程与网络线程的跨线程交互-源码解析)
    - [UpdateInNetThread 网络线程执行的操作](#updateinnetthread-网络线程执行的操作)
    - [UpdateInMainThread 主线程执行的操作](#updateinmainthread-主线程执行的操作)
    - [Callback 注册主线程网络回调逻辑](#callback-注册主线程网络回调逻辑)
    - [AService](#aservice)
    - [AChannel](#achannel)
  - [TCP](#tcp)
    - [TCP TService (网络线程驱动)](#tcp-tservice-网络线程驱动)
    - [TService 网络线程 Update](#tservice-网络线程-update)
    - [TService acceptor.AcceptAsync 新连接](#tservice-acceptoracceptasync-新连接)
    - [TService connect 操作](#tservice-connect-操作)
    - [TChannel 属性](#tchannel-属性)
    - [TChannel socket.ConnectAsync 连接](#tchannel-socketconnectasync-连接)
    - [TChannel accept 操作](#tchannel-accept-操作)
    - [TChannel socket.ReceiveAsync 接收数据](#tchannel-socketreceiveasync-接收数据)
    - [TChannel socket.SendAsync 发送数据](#tchannel-socketsendasync-发送数据)
    - [CircularBuffer 接收应用层缓存](#circularbuffer-接收应用层缓存)
    - [PacketParser 消息的解析 写入 recvBuffer 缓存](#packetparser-消息的解析-写入-recvbuffer-缓存)
    - [序列化](#序列化)
    - [反序列化](#反序列化)
    - [PacketParser.Parse 接收协议 recvBuffer 缓存](#packetparserparse-接收协议-recvbuffer-缓存)
    - [TChannel.Send 写入 sendBuffer缓存](#tchannelsend-写入-sendbuffer缓存)
    - [内网建立与另一个进程的连接](#内网建立与另一个进程的连接)
    - [遇到的问题](#遇到的问题)
  - [KCP](#kcp)
    - [TCP的问题](#tcp的问题)
    - [UDP特点](#udp特点)
    ...
    - [KCP 发消息流程](#kcp-发消息流程)
    - [KService](#kservice)
    - [KService 初始化 accept connect](#kservice-初始化-accept-connect)
    - [KService.Send 发送数据](#kservicesend-发送数据)
    - [KService.Recv 接收数据](#kservicerecv-接收数据)
    - [KService.Update 网络线程驱动](#kserviceupdate-网络线程驱动)
    - [KService.Recv SYN](#kservicerecv-syn)
    - [KService.Recv MSG](#kservicerecv-msg)
    - [KChannel](#kchannel)
    - [KChannel Connect Client 发起连接 SYN](#kchannel-connect-client-发起连接-syn)
    - [KChannel Accept Server 接收连接 读写消息](#kchannel-accept-server-接收连接-读写消息)
    - [KChannel.InitKcp 初始化 KCP参数](#kchannelinitkcp-初始化-kcp参数)
    - [KChannel HandleRecv 接收消息 MSG](#kchannel-handlerecv-接收消息-msg)
    - [KChannel KCP 消息发送 分片](#kchannel-kcp-消息发送-分片)
    - [KChannel Output KCP发送消息回调 MSG](#kchannel-output-kcp发送消息回调-msg)
  - [NetworkComponent](#networkcomponent)
    - [Session](#session)
    - [NetClientComponent 客户端网络组件](#netclientcomponent-客户端网络组件)
    - [NetServerComponent 服务端网络组件](#netservercomponent-服务端网络组件)
    - [NetInnerComponent 内网进程网络组件](#netinnercomponent-内网进程网络组件)
    - [NetInnerComponent CreateInner 内网连接](#netinnercomponent-createinner-内网连接)
    - [OpcodeTypeComponent 协议码组件](#opcodetypecomponent-协议码组件)
    - [MessageDispatcherComponent 消息分发组件](#messagedispatchercomponent-消息分发组件)
    - [SessionAcceptTimeoutComponent 会话超时判断组件](#sessionaccepttimeoutcomponent-会话超时判断组件)
    - [PingComponent](#pingcomponent)
    - [SessionIdleCheckerComponent](#sessionidlecheckercomponent)
    - [SessionMessageStatisticsComponent](#sessionmessagestatisticscomponent)
