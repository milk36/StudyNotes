...
[`is 运算符检查表达式的结果是否与给定的类型相匹配。`](https://learn.microsoft.com/zh-cn/dotnet/csharp/language-reference/operators/is)
...
using 语句可确保正确使用 `IDisposable` 实例; <big>确保正确使用可释放对象</big>
...
### [运算符的重载](https://learn.microsoft.com/zh-cn/dotnet/csharp/language-reference/operators/operator-overloading)
...
[`Activator.CreateInstance` 可能比 `new` 的慢几个数量级。](https://www.reddit.com/r/csharp/comments/djz7au/is_activatorcreateinstance_as_fast_as_new/)
