...
遇到 `error: NU1100: Unable to resolve` 可能是缺少Nuget源引起

您可以通过从项目或解决方案目录运行 `dotnet nuget list source` 来验证这一点。

[可以尝试使用这个方法](https://stackoverflow.com/questions/68283730/error-nu1100-unable-to-resolve-microsoftofficecore-15-0-0-for-net5-0)

1. 尝试使用 `dotnet nuget locals all --clear` 清除 NuGet 缓存，然后尝试添加包。
1. 如果这不起作用，请尝试删除 `C:\Users\<username>\AppData\Roaming\NuGet` 目录中存在的 `NuGet.config` 文件，然后使用 `dotnet restore` 命令恢复它。尝试在此之后添加你的包。
...
[配置邮件日志输出](https://github.com/nlog/NLog/wiki/Mail-target)
  ...
  ```xml
    ...
    <target xsi:type="Mail" name="InfoMail"
         ...
         />
    <rules>     
      <logger ruleName="ServerMail" name="*" minlevel="Erro" maxlevel="Fatal" writeTo="InfoMail" /> 
    </rules>
  ```
...
### Benchmark 基准测试(网络消息处理速率)

主要逻辑: `BenchmarkClient` 发送大量的 `C2G_Benchmark` 协议到 `BenchmarkServer`

* `BenchmarkServer` 启动参数 `--AppType=Server --StartConfig=StartConfig/Benchmark --Process=1`
* `BenchmarkClient` 启动参数 `--AppType=Server --StartConfig=StartConfig/Benchmark --Process=2`
