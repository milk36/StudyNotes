...
- [网络游戏架构设计 笔记二](#网络游戏架构设计-笔记二)
  - [配置](#配置)
    - [Option](#option)
    - [watcher 启动模式](#watcher-启动模式)
    - [Log](#log)
    - [Excel](#excel)
  - [Component 组件系统](#component-组件系统)
    - [Unit 单位](#unit-单位)
    - [为什么用组件](#为什么用组件)
    - [Unity 组件的优点/缺点](#unity-组件的优点缺点)
    - [ECS(指守望先锋的ECS)的问题](#ecs指守望先锋的ecs的问题)
  - [ET 的 ECS (Entity Component System)](#et-的-ecs-entity-component-system)
    - [ET中ECS的特性](#et中ecs的特性)
    - [ET组件数据/方法分离的好处](#et组件数据方法分离的好处)
    - [逻辑分发 - 多态](#逻辑分发---多态)
    - [Entity 树](#entity-树)
    - [最佳实践](#最佳实践)
    - [Entity 代码解读](#entity-代码解读)
    - [组件 调用`ComponentParent`方法](#组件-调用componentparent方法)
    - [区分需要序列化的组件](#区分需要序列化的组件)
  - [Child 与 Component 的区别](#child-与-component-的区别)
    - [Entity.AddChild](#entityaddchild)
    - [Entity.AddComponent](#entityaddcomponent)
  - [EventSystem 事件系统](#eventsystem-事件系统)
    - [Publish 发布事件](#publish-发布事件)
    - [Invoke](#invoke)
    - [Publish 跟 Invoke的区别](#publish-跟-invoke的区别)
    - [EventSystem Publish Invoke 源码解析](#eventsystem-publish-invoke-源码解析)
    - [`Publish` 同步/异步](#publish-同步异步)
    - [`Invoke` 无返回值/有返回值调用](#invoke-无返回值有返回值调用)
    - [EventSystem TypeSystems 源码解析](#eventsystem-typesystems-源码解析)
    - [EventSystem Awake](#eventsystem-awake)
