...
* 前端跟后端合并
  ...
  1. 不需要单独器服务端, 前后端代码共享更加方便

  1. 后端可以更方便做Unity可视化工具, 实体/组件可视化树
  
      开启方式: 选择 `ET->ChangeDefine` 菜单下的 `ENABLE_CODES` 和 `ENABLE_VIEW` 选项, 使用 `Client Server` 模式启动

      <img src="../img/ET7_EntityTreeWindow_1.png" width="30%"/>

  1. 更方便的插件机制 Excel和Protobuf工具等

  1. 竞技游戏 推图游戏 MMO(陪玩机器人) MOBA(陪玩机器人) 状态同步 帧同步(防作弊,预测回滚) `对前后端代码共享的必要性`
* 前后端合并细节
...
1. `MainThreadSynchronizationContext` 主线程驱动入口, 其中包含 `ThreadSynchronizationContext` 线程同步队列
   ...
   <big>这样设置 `SynchronizationContext` 同步上下文以后, 其他的IO异步操作完成后, 才能正确回调到主线程中</big>
1. 定义 `Task-Like` 类任务 类型

    <img src="../img/ET7_Await_1.png" width="70%">
...
* Id与InstanceId 的结构主要在时间内容有区别
  
  `IdStruct` 时间拼接用的是 `TimeSince2020()` 即2020开始至今的时间; 用业务逻辑属于永久性的id标识

  `InstanceIdStruct` 时间拼接用的是 `TimeSinceThisYear` 即今年开始至今的时间; instanceId 相对存在的生命周期要短很多,玩家离线后就没有意义了
* `GenerateUnitId` 主要是带个zone信息，方便获取; [默认可以都使用 `GenerateId`](https://et-framework.cn/d/1876-generateidgenerateunitid)
